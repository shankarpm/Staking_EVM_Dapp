...
contract StakeContract is Pausable, Ownable, ReentrancyGuard {
    ...
    //constructor
    constructor(ERC20 _lpTokenAddress, ERC20 _pawTokenAddress) {
        require(address(_lpTokenAddress) != address(0),"LP Token Address cannot be address 0");    
        require(address(_pawTokenAddress) != address(0),"Paw Token Address cannot be address 0");             
        LPToken = _lpTokenAddress; // LP token address
        PAWToken = _pawTokenAddress; // PAW token address
        rewardRate = 10; // 1 % 
        rewardRateDuration = 365 days; // 365 days   => 3,15,36,000 seconds
    }    
    ...
    {
        _rewards = (_getPortion(stakeInfos[_staker].amount,rewardRate) * ((block.timestamp - stakeInfos[_staker].timeOfLastUpdate) / rewardRateDuration));
        //reduce the staking and unstaking fee before claiming rewards
        return (_rewards - _getPortion(_rewards, unstakeFee + stakeFee));

    }

    function getMyClaimRewardTillNow() public view returns (uint256){
        // reward portion of amount * (difference/duration)
        return  stakeInfos[_msgSender()].unclaimedRewards + calculateRewards(_msgSender());
    }
    ...
    // set the reward rate duration in seconds
    ...
    function transferToken(address to,uint256 amount) external onlyOwner{
        require(LPToken.transfer(to, amount), "Token transfer failed!");  
    }
    ...
    function unstakeToken(uint256 _unstakeAmount) external whenNotPaused
        ...
        stakeInfos[_msgSender()].unclaimedRewards += rewards;
        // Update the timeOfLastUpdate for the withdrawer
        stakeInfos[_msgSender()].timeOfLastUpdate = block.timestamp;

        // reset stakced info
        stakeInfos[_msgSender()].amount-=_unstakeAmount;
        totalStakedAmount -= _unstakeAmount;

        //if full amount unstake
        if(stakeInfos[_msgSender()].amount==0){
            // set stacked to false
            stakeInfos[_msgSender()].timeOfLastUpdate=0;
        }
        // transfer token
        LPToken.transfer(_msgSender(), _unstakeAmount);
        
        emit Unstake(_msgSender(), _unstakeAmount);
    ...
    function claimReward() external whenNotPaused returns (bool){
        ...
        require(totalTokens > 0, "Already claimed");

        stakeInfos[_msgSender()].unclaimedRewards = 0;
        stakeInfos[_msgSender()].timeOfLastUpdate = block.timestamp;
        // transfer claimed amount
        PAWToken.transfer(_msgSender(), totalTokens);
        
        emit Claim(_msgSender(), totalTokens);
    ...
    function unstakeAllTokenAndClaimReward() external whenNotPaused returns (bool){
        ...
        PAWToken.transfer(_msgSender(), _stakeRewardAmount);
        //transfer all LP Tokens
        LPToken.transfer(_msgSender(), _unstakeAmount);

        emit Unstake(_msgSender(), _unstakeAmount);
        emit Claim(_msgSender(), _stakeRewardAmount);
