...
contract StakeContract is Pausable, Ownable, ReentrancyGuard {
    ...
    constructor(ERC20 _tokenAddress) {
        ...
        PAWToken = _tokenAddress; // PAW token address
        rewardRate = 10; // 1 % 
        rewardRateDuration = 365 days; // 365 days   
    }    
    ...
    {
        return (_getPortion(stakeInfos[_staker].amount,rewardRate) * ((block.timestamp - stakeInfos[_staker].timeOfLastUpdate) / rewardRateDuration));
    }
    ...
    function transferToken(address to,uint256 amount) external onlyOwner{
        require(PAWToken.transfer(to, amount), "Token transfer failed!");  
    }
    ...
    function stakeToken(uint256 _stakeAmount) external whenNotPaused 
        ...
        require(PAWToken.balanceOf(_msgSender()) >= _stakeAmount, "Insufficient Balance");
        
            PAWToken.transferFrom(_msgSender(), address(this), _stakeAmount);
    ...
    function unstakeToken(uint256 _unstakeAmount) external whenNotPaused
        ...
        _unstakeAmount = _unstakeAmount - _getPortion(_unstakeAmount,unstakeFee);
        ...
        // transfer token
        PAWToken.transfer(_msgSender(), _unstakeAmount);
    ...
    function claimReward() external whenNotPaused returns (bool){
        ...
        require(stakeInfos[_msgSender()].unclaimedRewards > 0, "Already claimed");
            
        // get claimed amount
        uint256 totalTokens = stakeInfos[_msgSender()].unclaimedRewards + calculateRewards(_msgSender());
    ...
    function unstakeAllTokenAndClaimReward() external whenNotPaused returns (bool){
        ...
        // transfer all tokens
        PAWToken.transfer(_msgSender(), (_unstakeAmount+_stakeRewardAmount));
